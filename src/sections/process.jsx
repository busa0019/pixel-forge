...
import { FaSearch, FaLightbulb, FaPencilRuler, FaCode, FaCheckCircle, FaPaperPlane } from "react-icons/fa";
import AOS from "aos";
  ...
  return (
    <section id="process" className="w-full py-20 px-6 md:px-16 lg:px-24 bg-gradient-to-b from-white to-gray-100">
      <div className="text-center mb-16" data-aos="fade-up">
        ...
        <p className="italic text-lg text-[#666] mb-4">"Great design is invisible; great experiences are unforgettable."</p>
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-8 flex flex-col justify-between transition-all duration-500 border border-gray-100 shadow-md hover:shadow-lg ${
                isCTA ? "bg-blue text-white hover:border-2 hover:border-white" : "bg-white hover:border-2 hover:border-blue"
              }`}
              style={{
                boxShadow:
            ...
            >
              <div className="flex items-center gap-4 mb-6">
                <div className={`p-4 rounded-xl ${isCTA ? "bg-white/20" : "bg-[#f5deb3]/20"}`}>{step.icon}</div>
                {!isCTA && <span className="text-4xl font-bold text-blue/20">{`0${step.id}`}</span>}
              </div>

              <div className={`${isCTA ? "flex-grow" : ""}`}>
                <h3 className={`text-xl boldText mb-3 ${isCTA ? "text-white" : "text-[#111]"}`}>{step.title}</h3>
